...
> ⚠️ **這份是歷史紀錄，不是現行規範。**
> 要寫新樣式請看 **[`DESIGN_GUIDELINES.md`](DESIGN_GUIDELINES.md)**。
>
> 本檔記錄的是「27 階字級併成 10 階」那一輪的盤點數據與合併理由，
> 寫於 V3 冷色調色盤導入**之前**。以下段落已被 V3 取代，僅供回溯：
>
> - **§5 顏色** —— 整節講的是舊的暖色系統（奶紙白 `#FAF6EF`、sage、clay）。
>   V3 已換成冷調 off-white + Navy / Cobalt / Mineral / Aqua / Amber / Sign。
> - **§5.1 的「已知的對比取捨」** —— 該問題 V3 已用 `--surface-accent`
>   在淺底自動換成 Mineral 700 解掉。
> - **§5.3** —— `--stop-1` ~ `--stop-4` 已不存在（現在只剩 `--stop-0-*`），
>   `site.js` 也不再讀取它們。圓環圖的顏色依賴仍然成立。
>
> §1–§4（字級／行高／字距／字重）與 §6（字型）、§7（修掉的 bug）仍然有效。
